...
class Resep extends Model
{
    ...
    public function get_bahan($id_resep){
        ...
        $cmd = "SELECT `nama_bahan`, `ukuran_jual`, `satuan_jual`, `ukuran_beli`, `satuan_beli`, `konv_beli`, IF(MOD(`konv_beli`,`ukuran_jual`) = 0,(`konv_beli` DIV `ukuran_jual`),(`konv_beli` DIV `ukuran_jual`) + 1) `rec_beli`, `harga` 
                FROM (SELECT `nama_bahan`, `ukuran_basic` `ukuran_jual`, b.`id_satuan` `satuan_jual`, `qty` `ukuran_beli`, b.`harga` `harga`, rb.`id_satuan` `satuan_beli`, (k.`value`*`qty`) `konv_beli` FROM `konversi` k, `bahan` b, resep_bahan rb WHERE b.id_bahan = rb.id_bahan AND rb.`id_satuan` = k.`satuan_awal` AND b.`id_satuan` = k.`satuan_hasil` AND rb.id_resep =:id_resep) b;";
